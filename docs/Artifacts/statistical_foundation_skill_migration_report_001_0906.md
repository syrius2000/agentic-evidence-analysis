...
計画書（[statistical_foundation_skill_migration_plan_001_0906.md](./statistical_foundation_skill_migration_plan_001_0906.md)）に沿って全 5 ステップの改修を完了した。
...
| 5 | `docs/reference/four_axis_cell_diagnostics.md` | 4 軸診断体系（Effect, Evidence, Influence, Stability）の数理的定義と Leverage 補正 Score 統計量。 |
| 6 | `docs/reference/README.md` | 統計リファレンス目録を新 4 軸体系へ整合（旧文書は Archives へ退避）。 |
| 7 | `.agents/skills/vcd-bayesian-evidence-analysis/templates/pass1_compute.R` | 9 モデル適合、明示式 BIC 算出、4 軸セル診断算出（$h \ge 0.80$ で隔離）、Dirichlet 事後推論関数を実装。2/3元表ガード追加。 |
| 8 | `.agents/skills/vcd-bayesian-evidence-analysis/templates/analysis.R` | 旧スコア算出を全廃。4 軸診断構造（`cells`）、モデル選択（`models`）を `evidence_results.json` および `dt_table.html` に出力。 |
| 9 | `.agents/skills/vcd-bayesian-evidence-analysis/Reference.md` | 旧エビデンススコア解説を撤廃し、4 軸および Leverage 補正 Score 統計量の数理的定義へ更新。 |
...
| 11 | `.agents/skills/vcd-bayesian-evidence-analysis/SKILL.md` | 旧 CLI オプション（`--threshold_k` 等）を削除し新 4 軸オプションへ改訂。4 変数例を 3 変数へ修正。 |
| 12 | `.agents/skills/vcd-bayesian-evidence-analysis/templates/dashboard.Rmd` | 統計カード、モデル比較表（明示式BIC）、Top-K 4軸表（対数P値追加・ソートキー整合）、全セルDT表、新用語解説へ全面刷新。 |
| 13 | `tests/scripts/vcd_bayesian_gen_executive_summary.R` | テスト用サマリー生成スクリプトを旧スコアから新 4 軸仕様へ改修。 |
...
| **安定セル率** | 16 / 16（100% REGULAR） | 16 / 16（100% REGULAR） | 数値的不安定セルなし（$h < 0.80$, $E \ge 5$） |
