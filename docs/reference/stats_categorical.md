...
ピアソン残差の分散は 1 ではなく、モデルの設計行列に依存して $1 - h_{ii}$ に縮小します（Haberman, 1973; Agresti, 2013）。したがって、レバレッジ補正された調整済み残差は次式で与えられます：

$$
r_{S,i} = \frac{O_i - E_i}{\sqrt{E_i (1 - h_{ii})}}
$$

ここで $h_{ii}$ はポアソン GLM におけるハット行列 $H$ の対角成分（Leverage）です。この残差の二乗は、セルごとの局所スコア統計量として $T_i^{\rm score} = r_{S,i}^2 = \frac{r_{P,i}^2}{1 - h_{ii}}$ と定義します（Rao, 1948; Pregibon, 1981）。

標準正規分布および自由度 1 のカイ二乗分布による解釈は、モデルが正則であり期待度数が十分に大きい場合の漸近近似です。観測ゼロ、小期待度数、過大 Leverage、境界推定を含むセルでは近似が破綻し得るため、`QUARANTINED` として P 値や $T_i^{\rm score}$ の閾値解釈を保留します。セルの隔離基準と大標本での探索的な Dual-Filter は、[3次元カテゴリカル探索の数理](three_way_models.md) の Stability および Evidence の節を参照してください。
...
小標本やセル数が多い分割表では、標本 Cramér's V は上方にバイアス（過大評価）を持ちます。Bergsma (2013) はカイ二乗統計量の漸近期待値 $df = (R - 1)(C - 1)$ を差し引いた**有限標本バイアス低減推定量（Bias-Reduced Estimator）** $\tilde{V}$ を提案しています：

$$
\begin{aligned}
  \tilde{\phi}^2 &= \max\left(0, \; \frac{X^2}{N} - \frac{(R - 1)(C - 1)}{N - 1}\right) \\[8pt]
  \tilde{R} &= R - \frac{(R - 1)^2}{N - 1}, \qquad \tilde{C} = C - \frac{(C - 1)^2}{N - 1} \\[8pt]
...
> [!NOTE]
> **推定量としての性質**:
> $\tilde{\phi}^2$ における $\max(0, \cdot)$ の非線形切断処理および $\tilde{V}$ への平方根変換を伴うため、数学的に期待値が母数と厳密に一致する「不偏推定量」ではありません。しかし、標本サイズ $N$ が中程度以下で自由度 $df$ が大きい表において、通常の $V$ が示す顕著な上方バイアスを大幅に低減・抑制する実務上極めて有効な補正推定量です。

#### 2元効果量と3次元分割表解析の境界
1. **適用単位の峻別**: Cramér's V は 2 次元分割表の全体関連度を要約する指標であり、3 次元分割表に一括して直接適用することはできません。適用する場合は「2 次元周辺表（例: $AB$ 周辺表）」あるいは「第 3 変数 $C$ の各層別 2 次元表」ごとに独立して算出します。
2. **周辺化とシンプソンの乖離**: 周辺表から求めた $V_{AB}^{(\mathrm{marginal})}$ と、各層 $k$ における層別 $V_{AB \mid k}$ は一般に一致しません。交絡が存在する場合、周辺化によって見かけ上の相関が増大・消失・逆転（Simpson 型の現象）するため、周辺 $V$ だけで関連性を評価することは危険です。
3. **セル単位の評価**: 3 次元表のセル単位の偏りは、表全体の効果量である $V$ ではなく、局所対数効果比 $\log(O_i / E_i)$ や大標本規格化差 $e_i^{(\mathrm{global})}$ などの 4 軸セル診断指標で評価します。

---

## 4. 旧Evidence Score（$r^2-k\log N$）の限界と監査専用列化

過去のプロトタイプでは、未補正残差二乗に罰則を引く $r^2-k\log N$（$k=1$または$k=2$）が検討されました。この式は、再適合したセル追加モデルの尤度差やパラメータ差から導かれた局所BIC差ではありません。度数を100倍（$N \to 100N$）にする実験では、固定した非ゼロ乖離に対して残差二乗が標本数のオーダーで増え、罰則は$\log N$のオーダーでしか増えません。
...
1. **候補判定への不適合**:
   ...
   このため、正値を候補判定へ使うと、十分大きなNで小さな乖離まで通過しやすくなります。ただし、全セルが必ず正値になるとは限らず、統計的証拠が増える現象そのものは数学的異常ではありません。
2. **局所尤度比検定（LRT）との乖離**:
   $r^2 - k\log N$ は、セルダミーを追加した対数線形モデルの再適合による逸脱度改善量 $\Delta G_i^2$ や局所 BIC 差分 $\Delta G_i^2 - \Delta p\log N$ と厳密に一致しません。

このため、現行ツールキットでは旧スコアを**監査専用列（audit-only）**としてのみ保持し、真の信号判定・セル合否判定・セルBayes factorの代用としては使用しません。局所モデル比較が必要な場合は、比較する基準モデルとセル追加モデルを明示し、同じデータ・支持集合・尤度の下で再適合した$\Delta G_i^2$または対応する局所BIC差を用います。Rao scoreは基準適合だけから同じ追加方向を検定する近似であり、有限標本での等式や正確なBayes factorではありません。
