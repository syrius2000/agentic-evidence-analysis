...
を評価します。同時事後分布 $\operatorname{Dirichlet}(\boldsymbol{y} + \boldsymbol{\alpha})$ からのモンテカルロ・サンプリング（Monte Carlo draws）により、層間差 $\Delta \theta = \theta_{y \mid g_1} - \theta_{y \mid g_2}$ の完全な事後分布を標本化し、点ごとの 95% 等裾信用区間（Equal-Tailed Interval: ETI）を導出します（※ 本ツールキットの正本実装 `bayes.R` では ETI を採用・永続化しています。最高事後密度区間 HDI は非対称分布における理論的選択肢として位置付けられます）。

### 3.4 部分集合分子・分母による一般化条件付き割合と事後差

仕様 `conditional-rate-view` では、多水準の応答変数から関心のある水準の部分集合を分子・分母として柔軟に指定できる一般化条件付き割合を定式化しています：

- 応答変数 $Y$ の全水準集合を $\mathcal{Y}$
- 分子水準集合 $A \subseteq \mathcal{Y}$、分母水準集合 $B \subseteq \mathcal{Y}$（ただし $A \subseteq B$ かつ $B \neq \emptyset$）
- 層別変数 $G$（組合せ水準 $g$）

このとき、層 $g$ における一般化条件付き割合は次式で定義されます：

$$
\theta_{A \mid B, g} = \frac{\sum_{y \in A} \pi_{g, y}}{\sum_{y \in B} \pi_{g, y}}
$$

#### 層間差（Rate Difference）の同時事後推論
比較対象の 2 つの層 $g_1, g_2$（例えば Gender: Male vs Female、あるいは特定 Dept 内での比較）に対する割合の差：

$$
\Delta \theta = \theta_{A \mid B, g_1} - \theta_{A \mid B, g_2}
$$

の不確実性を評価する場合、独立した 2 つの二項事後分布を別々に近似するのではなく、**全セル同時事後分布 $\boldsymbol{\pi}^{(s)} \sim \operatorname{Dirichlet}(\boldsymbol{y} + \boldsymbol{\alpha})$（$s = 1, \ldots, S$）から同一のドローを用いて計算**します：

$$
\Delta \theta^{(s)} = \frac{\sum_{y \in A} \pi_{g_1, y}^{(s)}}{\sum_{y \in B} \pi_{g_1, y}^{(s)}} - \frac{\sum_{y \in A} \pi_{g_2, y}^{(s)}}{\sum_{y \in B} \pi_{g_2, y}^{(s)}}
$$

これにより、セル間の確率総和制約（$\sum \pi_i = 1$）に伴う負の相関（多項共分散構造）が完全に保持され、点ごとの区間を不当に狭めたり広げたりしない厳密な 95% 信用区間（ETI）が算出されます。

#### 分母ゼロと部分 HOLD の数理
観測データにおいて分母水準集合 $B$ の度数合計がゼロ（$\sum_{y \in B} y_{g, y} = 0$）である場合、最尤割合は $0/0$（未定義）となります。事前分布 $\alpha$ を加えた事後平均は計算可能ですが、事前集中度 $\alpha$ の感度が極めて強く支配し、事後分散が急拡大します。
このため、本システムでは分母度数がゼロの層について推測値を補完せず、**`PARTIAL_HOLD`（部分保留）** として理由を明示し、独立して成立するモデル比較や他の層の診断と分離して報告する契約を徹底しています。

### 3.5 対数線形モデルのオッズ比不変性とシンプソンのパラドックス

3 元分割表における対数線形モデル（M1〜M9）と条件付き割合・オッズ比には、極めて深い数理的対応が存在します：

#### 均一連関モデル M8 におけるオッズ比不変性
均一連関モデル M8 $[AB][AC][BC]$ では、3 次交互作用項 $\lambda_{ijk}^{ABC} = 0$ と仮定されます。このとき、$2 \times 2 \times K$ 分割表（または多水準表から抽出した任意の $2 \times 2$ 部分表）において、第 3 変数 $C$ の層 $k$ を条件付けたときの局所条件付きオッズ比（Conditional Odds Ratio）は：

$$
\psi_{AB \mid k} = \frac{\mu_{11k} \mu_{22k}}{\mu_{12k} \mu_{21k}} = \exp\left( \lambda_{11}^{AB} + \lambda_{22}^{AB} - \lambda_{12}^{AB} - \lambda_{21}^{AB} \right)
$$

となります。右辺の対数交互作用対比（Interaction Contrast）は、基準水準制約（コーナーストーン制約 $\lambda_1 = 0$）や和ゼロ制約（$\sum \lambda = 0$）などのパラメータ化（identifiability constraint）に依存しない不変量であり、**第 3 変数の層 $k$ に依存せずすべての層で一定（Homogeneous Odds Ratio）** となります。均一連関とは「2 変数間の連関の強さが、第 3 変数のどの水準で見ても不変である」という構造を表します（※ 構造ゼロや境界推定セルが存在する場合はオッズ比が未定義または発散することに留意）。

#### シンプソンのパラドックス（Simpson's Paradox）の数理的解消
周辺分割表（$AB$ 2 元表）で算出される周辺オッズ比 $\psi_{AB}^{\text{marginal}}$ と、層別条件付きオッズ比 $\psi_{AB \mid k}$ は一般に一致しません。周辺表では強い正の関連を示しているにもかかわらず、交絡変数 $C$ で層別すると全層で関連が消失（$\psi_{AB \mid k} = 1.0$、M5 条件付き独立モデル）、あるいは逆転する現象がシンプソンのパラドックスです（Yule, 1903; Simpson, 1951）。
本ツールキットの 3 次元探索経路は、大域的対数線形モデル比較（M5 条件付き独立 vs M8 均一連関）と、条件付き割合ビュー（`conditional-rate-view`）を連携させることで、交絡による見かけの連関を統計的な条件付き関連として分離・可視化できるように設計されています（※ これは統計的モデル上の層別化・共変量調整であり、未観測交絡の不在などの因果的同定を自動保証するものではありません）。
...
一方、独立モデル $M_1$ の周辺尤度は、共通の多項尤度において独立性仮定 $\pi_{ijk} = \pi_{i\cdot\cdot}^A \pi_{\cdot j\cdot}^B \pi_{\cdot\cdot k}^C$ を課し、各周辺 Dirichlet 事前分布（ハイパーパラメータ $\boldsymbol{\alpha}^A, \boldsymbol{\alpha}^B, \boldsymbol{\alpha}^C$）に関して積分することで得られます。重要な点として、独立モデルの周辺尤度は**共通の 1 つの多項係数 $\frac{N!}{\prod n_{ijk}!}$ に対し、3 つの周辺 Dirichlet 積分を乗じた形**となります（多項係数を 3 回掛けるのではない点に注意）：

$$
p(\boldsymbol{y} \mid M_1) = \frac{N!}{\prod_{i,j,k} n_{ijk}!} \times \left[ \frac{\Gamma(\alpha_0^A)}{\Gamma(N + \alpha_0^A)} \prod_i \frac{\Gamma(n_{i\cdot\cdot} + \alpha_i^A)}{\Gamma(\alpha_i^A)} \right] \times \left[ B \text{ 周辺積分} \right] \times \left[ C \text{ 周辺積分} \right]
$$

したがって、飽和モデルと独立モデルの厳密な対数ベイズ因子 $\log BF_{91} = \log p(\boldsymbol{y} \mid M_9) - \log p(\boldsymbol{y} \mid M_1)$ を算出する際、共通の多項係数 $\frac{N!}{\prod n_{ijk}!}$ は完全に相殺されます。これにより、対数ガンマ関数 `lgamma()` の差分計算のみで桁あふれなく厳密値が得られます（実装 `bayes.R` の `calc_dirichlet_bf()` と一致）。
...
> [!NOTE]
> **PPP-value の診断的解釈と制約**:
> 1. **中心付近（PPP ≈ 0.50）の解釈**: PPP が 0.50 付近にあることは、選定された検定統計量 $T(y, \pi)$ に関して観測データが事後予測分布の中央付近に位置していることを示す診断的目安であり、モデル全体の正しさや完全適合、構造の正当性を証明するものではありません。
> 2. **極端値（< 0.05 または > 0.95）**: 0.05 未満または 0.95 超の極端な値は、事前分布の不整合やモデルの不一致（過少適合・過適合）の兆候として扱いますが、固定的な合否境界ではなく診断上の注意喚起です。
> 3. **対象モデルの境界**: 本ツールキットでの PPP 計算は**飽和 Dirichlet 事後モデルに対する事後予測チェック**であり、対数線形構造モデル（M1〜M8）の適合性を保証するものではありません（実装 `bayes.R` / `render_report.R` の契約と整合）。
...
7. **Yule, G. U. (1903)**. "Notes on the theory of association of attributes in statistics." *Biometrika*, 2(2), 121–134. [DOI:10.1093/biomet/2.2.121](https://doi.org/10.1093/biomet/2.2.121)
8. **Simpson, E. H. (1951)**. "The interpretation of interaction in contingency tables." *Journal of the Royal Statistical Society: Series B (Methodological)*, 13(2), 238–241. [DOI:10.1111/j.2517-6161.1951.tb00088.x](https://doi.org/10.1111/j.2517-6161.1951.tb00088.x)
