...
本ツールキットでは、大標本環境における探索的セル抽出として以下の **Dual-Filter プロトコル** を適用します：
...
**旧エビデンススコア（$r^2 - k\ln N$）の破綻と監査列化**:
過去のプロトタイプで検討された旧指標 $r^2 - k\ln N$ は、大標本下で全セルが正値化（エビデンス飽和）してスクリーニング機能を失うため、現行システムでは監査専用列（audit-only）として隔離し、上記 Dual-Filter を真の抽出ロジックとして採用しています。
