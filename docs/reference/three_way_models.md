...
## 5. 3次元セル診断の探索的Dual-Filter

### 5.1 スクリーニング手順

固定した非ゼロ乖離では、標本サイズが大きくなるほど$T_i^{\rm score}$は増加し、P値は小さくなります。3次元の現行候補条件にNカットオフは含めません。候補は基準モデルに対する探索的な着目セルであり、実務的重要性や多重性調整済み結論ではありません。

```
【3次元 Dual-Filter スクリーニング手順】
...
> [!NOTE]
> **旧Evidence Score（$r^2-k\log N$）の監査列化**:
> 旧式は未補正残差二乗から直接作られ、セル追加モデルの再適合による$\Delta G_i^2$、局所BIC差、Rao scoreと同一ではありません。固定した非ゼロ乖離では正値になりやすくなるため、正値を候補抽出・セル合否へ使いません。全セルが必ず正値になるとは限らず、統計的証拠の増加自体を異常とは扱いません。
...
| **Audit（旧指標）** | 旧Evidence Score | $r_{P,i}^2-k\ln N$ | $c\cdot r_{P,i}^2-k(\ln N+\ln c)$ | $\mathcal{O}(N)$ | 固定した非ゼロ乖離では正値になりやすい。局所BIC差・Rao scoreの代理ではない。 |
