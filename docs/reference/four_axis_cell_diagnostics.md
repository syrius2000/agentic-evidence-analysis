...
1. **スパース性（極小度数）**: 期待度数 $E_i < 5$ の場合、ポアソン分布の離散性が顕著になり、連続分布近似が歪む。
2. **過大レバレッジ**: $h_{ii} \ge 0.8$ の場合、セルがモデル推定値の大部分を支配しており、有効情報量 $E_i (1 - h_{ii}) \to 0$ となり、推定分散が過大化・数値的に不安定化する。

### 5.2 診断状態（Stability Status）判定ルール

$$\mathrm{stability\_status}_i = \begin{cases} \mathbf{REGULAR} & (E_i \ge 5.0 \ \land \ h_{ii} < 0.8) \\ \mathbf{QUARANTINED} & (E_i < 5.0 \ \lor \ h_{ii} \ge 0.8) \end{cases}$$

- **`REGULAR`**: 大標本漸近正規性・カイ二乗近似が完全に成立。統計的推論および P 値の信頼性が極めて高い。
- **`QUARANTINED`**: 隔離・注意セル。統計量が極端に振れる可能性があるため、解釈を保留するか、またはベイズ Dirichlet 事後推論による厳密な信用区間を参照する。
