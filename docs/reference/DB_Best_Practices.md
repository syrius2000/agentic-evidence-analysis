# 分析入力データ契約: DB由来集計表の整合性

created: 2026-09-06 23:55 (JST)
update: 2026-09-12 21:54 (JST)
author: Codex (GPT-5) / Antigravity

このドキュメントは、MySQL / MariaDB などの RDBMS からカテゴリカル分析パイプライン（Pass 0〜Pass 3）へ集計表を受け渡す際の**最小データ契約**をまとめた補助リファレンスです。DBMS の設計、接続実装、SQL 実行、および運用の正本は `rwd-mysql-skill-toolkit` が担い、本リポジトリでは統計計算に必要な入力整合性だけを扱います。
...
- **日付・時刻**:
  時刻瞬間を表す列は、保存基準（原則 UTC）、接続セッションのタイムゾーン、およびレポート表示時の JST 変換をデータ契約で明示します。MySQL の `TIMESTAMP` は接続セッションのタイムゾーンに応じて UTC との変換対象になり得る一方、`DATETIME` はタイムゾーン情報を保持しない暦時刻です。両者を単に「JST の列」として同一視してはなりません。現地の壁時計時刻のみを意味する `DATETIME` は、対象地域・夏時間・変換要否を別途定義します。
