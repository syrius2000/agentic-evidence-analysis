...
resolve_pass3_run_dir <- function(root, required_filename, skill_label = "output") {
  ...
  run_dirs <- run_dirs[startsWith(basename(run_dirs), "run_")]
  required_paths <- file.path(run_dirs, required_filename)
  completed <- file.exists(required_paths)
  run_dirs <- run_dirs[completed]
  required_paths <- required_paths[completed]
  if (length(run_dirs) == 0L) {
    stop(
      "[ERROR] ", required_filename, " が見つかりません。期待ディレクトリ: ", root,
      "（直下）または ", root,
      "/run_<JST日時|slug>[_N]/ 。完了済みrunのout_rootまたはrun_output_dirを渡してください。"
    )
  }
  artifact_info <- file.info(required_paths)
  dir_info <- file.info(run_dirs)
  pick_order <- order(
    as.numeric(artifact_info$mtime),
    as.numeric(dir_info$mtime),
    basename(run_dirs),
    decreasing = TRUE
  )
  pick <- run_dirs[pick_order[1L]]
