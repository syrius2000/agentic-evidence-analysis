...
- **3次元・整数の集計度数**: 以下の新経路を使用する。目的変数未指定でも解析でき、指定時は条件付き割合を追加する。RD/RR/ORなどの効果量は明示的な結果契約がある場合に限って表示し、条件付き割合や残差から代用しない。
...
4. **ベイズの不確実性**: 条件付き割合の主事前は各セルα=0.5（多項Jeffreys）、感度はα=1.0。総事前濃度はKα。分母、点ごとの95%等裾信用区間、感度差を説明。セル確率の事後と対数線形モデルの事後を混同しない。旧α=1.0成果物をJeffreysと再ラベルしない。
