...
model_meanings <- c(M1="相互独立",M2="AB関連のみ",M3="AC関連のみ",M4="BC関連のみ",
 M5="BとCはAを条件として独立",M6="AとCはBを条件として独立",M7="AとBはCを条件として独立",
 M8="全2因子交互作用、3因子交互作用なし",M9="飽和モデル")
model_conditional_independence <- c(M5="B ⟂ C | A",M6="A ⟂ C | B",M7="A ⟂ B | C")
metric_ontology <- list(
  oe_ratio=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  log_oe_ratio=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  pearson_residual_per_sqrt_n=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  signed_deviance_per_sqrt_n=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  signed_deviance_sq_per_n=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  local_score_chisq=list(metric_class="evidence",sample_size_behavior="sensitive",inferential_status="asymptotic"),
  local_delta_g2=list(metric_class="evidence",sample_size_behavior="sensitive",inferential_status="asymptotic"),
  delta_g2_per_n=list(metric_class="magnitude",sample_size_behavior="invariant",inferential_status="descriptive"),
  leverage=list(metric_class="influence",sample_size_behavior="invariant",inferential_status="descriptive"),
  stability_status=list(metric_class="stability",sample_size_behavior="model_dependent",inferential_status="not_evaluated"),
  log_bf_bic_approx=list(metric_class="evidence",sample_size_behavior="sensitive",inferential_status="asymptotic"),
  exact_table_bf=list(metric_class="evidence",sample_size_behavior="sensitive",inferential_status="exact_prior_dependent"),
  posterior_cell_probability=list(metric_class="uncertainty",sample_size_behavior="sensitive",inferential_status="exact_prior_dependent"),
  practical_relevance_status=list(metric_class="practical_relevance",sample_size_behavior="model_dependent",inferential_status="not_evaluated")
)
...
fit_models <- function(df) {
  ...
  models <- lapply(names(fits), function(id) {
    ...
    if(is.null(f)) return(list(id=id,status=item$status,computation_status="FAILED",model_fit_status=item$status,
      chi_square_inference_status="HOLD",bic_approximation_status="HOLD",stability_status="NOT_EVALUATED",status_reasons=item$reasons))
    ...
    list(id=id,structure=model_structures[[id]],formula=paste(deparse(formula(f)),collapse=""),meaning=model_meanings[[id]],
      conditional_independence=if(id %in% names(model_conditional_independence)) model_conditional_independence[[id]] else NULL,
      status=item$status,status_reasons=item$reasons,warnings=item$warnings,
      computation_status="COMPUTED",model_fit_status=item$status,
      chi_square_inference_status=inference$status,bic_approximation_status=if(regular) "REGULAR_LAPLACE" else "HOLD",
      ...
      bic_stats_rows=BIC(f),fitted_values=unname(fitted(f)),inference=inference)
  ...
  for (small in names(fits)) for (large in names(fits)) {
    ...
    chi_square_ok <- regular && all(fitted(a$fit)>=5) && all(fitted(b$fit)>=5)
    bic_approximation_ok <- regular
    inference <- chi_tail(delta,dr,chi_square_ok)
    comparisons[[length(comparisons)+1L]] <- list(id=paste(small,large,sep="_to_"),baseline=small,alternative=large,
       ...
       log_bf_bic_approx=if(bic_approximation_ok) (delta-dr*log(N))/2 else NULL,
       model_fit_status=if(regular) "REGULAR" else "HOLD",
       chi_square_inference_status=inference$status,
       bic_approximation_status=if(bic_approximation_ok) "REGULAR_LAPLACE" else "HOLD",
       approximation_status=if(chi_square_ok) "REGULAR_ASYMPTOTIC" else "HOLD",
       stability_status="NOT_EVALUATED",inference=inference)
  }
...
cell_diagnostics <- function(df, fits, base_id) {
  ...
  cells <- lapply(seq_len(K),function(i) {
    ...
    chi_square_ok <- valid && all(mu>=5) && all(fitted(extra$fit)>=5)
    bic_approximation_ok <- valid
    inference <- chi_tail(if(valid) delta else NA_real_,dr,chi_square_ok)
    diagnostic_flags <- list(zero_observed=df$n[i]==0,sparse_expected=mu[i]<5,high_leverage=h[i]>=0.80)
    list(cell_id=sprintf("cell_%03d",i),levels=lapply(df[i,1:3],as.character),
      ...
      pearson_residual_per_sqrt_n=unname(rp[i]/sqrt(N)),signed_deviance_per_sqrt_n=unname(rd[i]/sqrt(N)),
      signed_deviance_sq_per_n=unname(rd[i]^2/N),rate_difference=unname((df$n[i]-mu[i])/N),
      ...
      local_bic_difference=if(bic_approximation_ok) delta-dr*log(N) else NULL,
      local_log_bf_bic_approx=if(bic_approximation_ok) (delta-dr*log(N))/2 else NULL,
      local_fit_status=if(valid) "REGULAR" else "HOLD",local_fit_reason=reason,
      estimation_status=if(valid) "REGULAR" else "HOLD",
      diagnostic_status=if(any(unlist(diagnostic_flags))) "REVIEW" else "REGULAR",diagnostic_flags=diagnostic_flags,
      chi_square_inference_status=inference$status,
      bic_approximation_status=if(bic_approximation_ok) "REGULAR_LAPLACE" else "HOLD",
      approximation_status=if(chi_square_ok) "REGULAR_ASYMPTOTIC" else "HOLD",
      stability_status="NOT_EVALUATED",inference=inference)
