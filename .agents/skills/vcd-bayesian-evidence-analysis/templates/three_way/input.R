...
read_config <- function(path) {
  ...
  inspection_sha256 <- if (!is.null(inspection$file_sha256)) inspection$file_sha256 else inspection$input_sha256
  if (!scalar_text(inspection_sha256) || !identical(inspection_sha256, cfg$consultation$input_sha256)) fail("検分の入力ハッシュ不一致。Pass 0を再実行")
  cfg
}
