...
modelrows<-lapply(r$models,function(m)list(m$id,m$structure,m$meaning,m$model_fit_status,m$deviance,m$df_residual,m$bic_multinomial_n,m$chi_square_inference_status,m$inference$log_p_value))
parts<-c(parts,'<section><h2>関連構造を比較する</h2><p>A・B・Cは上記変数の順です。BICは固定総度数の多項尤度と自由パラメータ数を使用。小さい値を比較の参考にしますが、自動的な勝者は決めません。</p>',table_html(c('モデル','構造','意味','推定状態','逸脱度G²','残差df','多項BIC(N)','χ²推論状態','適合検定 log P'),modelrows),'<details><summary>入れ子モデルの比較</summary>',table_html(c('基準→拡張','ΔG²','Δdf','ΔG²/N','近似log BF','log P','χ²推論状態','BIC近似状態'),lapply(r$comparisons,function(x)list(x$id,x$delta_g2,x$delta_rank,x$delta_g2_per_n,x$log_bf_bic_approx,x$inference$log_p_value,x$chi_square_inference_status,x$bic_approximation_status))),'</details><p>log Pはカイ二乗による漸近値です。小期待度数・非正則推定では保留します。期待度数5の条件はχ²推論の保守的heuristicでありBICの定義ではありません。BIC差による近似log BFを厳密BFやモデル事後確率と同一視しません。</p></section>')
...
for(base in names(r$cells)) {
 ...
 parts<-c(parts,'<details><summary>全セルの数値と推定状態</summary>',table_html(c('セル','A','B','C','O','E','log(O/E)','Pearson/√N','d/√N','d²/N','局所ΔG²','ΔG²/N','score統計量','leverage','診断状態','局所状態','χ²推論状態','BIC近似状態','Stability'),lapply(cs,function(c)list(c$cell_id,c$levels$A,c$levels$B,c$levels$C,c$observed,c$expected,c$log_oe_ratio,c$pearson_residual_per_sqrt_n,c$signed_deviance_per_sqrt_n,c$signed_deviance_sq_per_n,c$local_delta_g2,c$delta_g2_per_n,c$local_score_chisq,c$leverage,c$diagnostic_status,c$local_fit_status,c$chi_square_inference_status,c$bic_approximation_status,c$stability_status))),'</details><p>局所ΔG²はセルダミーを追加して再適合した改善量です。score統計量は局所近似、leverageはモデル行列の診断です。再標本化によるStabilityは未評価です。</p></section>')
}
post<-r$posterior
