...
3-way 以上の表を周辺化して本スキルで代替解析してはならない。条件付き独立性、交互作用、モデル比較が必要な場合は、正本スキル `vcd-bayesian-evidence-analysis` を使用する。

## 4. モデル選択の指針

1. **主効果モデルで p < 0.05** → 2次元の全体関連を示す探索的証拠であり、効果量・不確実性と併記する
2. **調整残差の絶対値 ≥ 1.96** → 多重性未調整の局所乖離の探索目安であり、Quarantine・期待度数・効果量を併記する
