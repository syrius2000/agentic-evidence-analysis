...
- 調整標準化残差ヒートマップ、条件付きestimand図、セル診断表

現行Dashboardではモザイク図を主表示・必須成果物とせず、Haberman型の調整標準化残差ヒートマップを局所診断の主表示とします。調整残差は独立モデルからの統計的乖離であり、P値、効果量、局所診断、不確実性は別々の情報として解釈します。
