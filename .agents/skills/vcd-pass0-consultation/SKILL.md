...
`<project>` と `<id>` は実際の識別子へ置き換え、通常実行ごとに新しいrun directoryを指定する。既存runへ無言で上書きしない。
`--out-dir=`、`--out-dir ""`、空の第2位置引数など、空のout-dirは拒否し、ファイルシステムのrootへ書き込まない。
...
```bash
Rscript .agents/skills/vcd-bayesian-evidence-analysis/templates/analysis.R --config output/project/run_v1/analysis_config.json
```
