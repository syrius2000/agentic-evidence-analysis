...
if (file.exists(summary_csv)) {
  ...
  if (all(cram_cols %in% names(s))) {
    ...
    signal_ok <- !is.na(s$marginal_strata_signal) &
      s$marginal_strata_signal %in% c("none", "review_stratified")
    check("marginal_strata_signal is always none or review_stratified", all(signal_ok))

    if (nrow(row2) == 1L) {
      check(
        "UCBAdmissions 3-way signal requests stratified review",
        identical(row2$marginal_strata_signal[1], "review_stratified")
      )
      check(
        "UCBAdmissions note records the 0.05 threshold",
        !is.na(row2$marginal_strata_note[1]) &&
          grepl(">= 0\\.05", row2$marginal_strata_note[1])
      )
    }
  }
}
