...
sec_agent <- file.path(ws, ".agents", "skills", "security-vulnerability-check")
if (!dir.exists(sec_agent)) {
  message("SKIP: security-vulnerability-check not in .agents/skills (see README.md).")
  quit(status = 0L)
}
...
# --- 両ディレクトリのベースパス ---
dirs <- c(
  file.path(ws, ".agents", "skills", "security-vulnerability-check")
)

if (!any(dir.exists(dirs))) {
  cat("[SKIP] security-vulnerability-check skill not found under .agents/.cursor; skipping reference checks.\n")
  quit(status = 0)
}
