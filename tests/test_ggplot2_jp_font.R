...
repo_root <- function() {
  ...
  if (!file.exists(file.path(d, ".agents")) && identical(basename(d), "tests")) {
...
vcd_rmd <- file.path(root, ".agents", "skills", "vcd-categorical-analysis", "templates", "dashboard.Rmd")
quest_rmd <- file.path(root, ".agents", "skills", "questionnaire-batch-analysis", "templates", "dashboard.Rmd")
