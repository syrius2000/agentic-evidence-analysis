# deterministic-r-dependencies Specification
...
- **THEN** 外部取得エラーを起こすことなく、正規回帰テストスイート（`tests/run_regression_suite.R` に登録された正規回帰テスト群）および主要解析スクリプト（Pass 1 統計計算）が決定論的に完了する
