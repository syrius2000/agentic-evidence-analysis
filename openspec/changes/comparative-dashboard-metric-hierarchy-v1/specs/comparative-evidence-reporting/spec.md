...
- **THEN** the E100 cell MUST display the natural-unit translation derived only from `excess_per_100` (for example `+3.20 / 100人`), and the adjacent NNT/NNH-like cell MUST independently display the reciprocal label according to domain and `reciprocal_direction` without combining both metrics into a single cell
...
#### Scenario: Retaining U-Grade with cell-local practical-region color

- **WHEN** `primary_delta` is configured and U-Grades U0–U3 (or NONE when delta is null) are present
- **THEN** the Practical Region / U-Grade column MUST remain in the twelve-column table, U-Grade values MUST continue to display with dominant region, and practical-region hue MUST be confined to that cell (U3 remaining muted/achromatic per existing contract)
...
- **THEN** CSV export MUST still serialize exactly the 40 canonical summary fields (excluding presentation-only keys), and `excess_per_100`, `reciprocal_absolute_rd`, `reciprocal_status`, and `reciprocal_direction` MUST NOT be added to Gower clustering feature keys
...
4. NNT-like / NNH-like labels are direction-aware Safety-domain presentation labels only when `reciprocal_status = "STABLE_DIRECTION"`
...
- **THEN** the guide MUST present RD, E100, and reciprocal RD as distinct hierarchical roles (primary contrast / natural-unit translation / secondary interpretation), MUST warn against causal NNT misinterpretation, and MUST remain aligned with Markdown report wording for the active inferential semantics
