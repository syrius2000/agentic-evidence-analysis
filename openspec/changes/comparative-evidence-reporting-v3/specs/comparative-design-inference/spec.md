...
The system SHALL evaluate 1:k matched-set cohort data (one treated subject and $k_j \ge 1$ controls per matched set $j=1,\dots,J$, without replacement) using atomic matched-set cluster bootstrap resampling targeting the ATT estimand, where each replicate computes set-weighted risks $\hat{p}_T = \frac{1}{J}\sum_{j=1}^J Y_{Tj}$ and $\hat{p}_R = \frac{1}{J}\sum_{j=1}^J \left(\frac{1}{k_j}\sum_{\ell=1}^{k_j} Y_{Rj\ell}\right)$, generating uncertainty replicates with `inferential_semantics = "bootstrap"` and `interval_method = "bootstrap_percentile"`.
...
- **THEN** the system MUST resample entire matched sets atomically with replacement, compute set-weighted marginal proportions $\hat{p}_T$ and $\hat{p}_R$, evaluate post-match balance (SMD), and emit the replicate distribution with `inferential_semantics = "bootstrap"`, `interval_method = "bootstrap_percentile"`, and `bootstrap_support_fraction_rd_gt_zero`.

### Requirement: IPTW Propensity Score Bootstrap with In-Replicate Model Refitting and Exact Weight Formulas

The system SHALL perform IPTW-adjusted comparative inference via patient-level bootstrap resampling that refits the propensity score model inside *every* bootstrap replicate (`iptw_mode = "refit_ps"`), computing exact weights according to the specified estimand:
...
while applying configured percentile weight truncation, recording effective sample size (ESS), maximum weight, and covariate balance diagnostics, and assigning `inferential_semantics = "bootstrap"` and `interval_method = "bootstrap_percentile"`.
