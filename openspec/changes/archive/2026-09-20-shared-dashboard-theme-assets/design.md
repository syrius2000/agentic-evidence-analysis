...
- 表示共通化時点では2次元α=0.5・3次元α=1.0という実際の生成条件を正しく表示する。その後の独立した工程で3次元 `compute_conditional_rate_view()` の主事前をα=0.5に変更し、α=1.0を感度分析に用いる。旧成果物はα=1.0のまま識別する。移行計画は `docs/Archives/20260920_154500_007/sources/implementation_plan_008_0919.md` に定める。
...
条件付き割合・群間差・主／感度の比較は保存前の固定小数丸めを廃止し、R倍精度の要約値から計算する。結果JSON書き出しには `digits=NA` を指定し、保存時の再丸めを防ぐ（他の指標の既存明示丸めは保持）。表示のみ百分率・有効桁で整形し、小さい非ゼロ値を0へ丸めない。ETI列名と図の水準は設定値から生成する。用語集冒頭も同じ構成比でのc倍と一般の追加標本を区別する。既存保存runは再ラベル・上書きしない。
