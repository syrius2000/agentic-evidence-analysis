...
## 5. 独立レビューF1〜F3の修正（Codexへの実施承認済み）

- [x] 5.1 条件付き割合・群間差・感度差の固定小数丸めとJSON再丸めを解消し、結果契約を同期する。ゼロ・希少・大N希少・集約率を実関数とBeta解析式で照合し、全ゼロHOLDとJSON往復を確認する
- [x] 5.2 表のETI列名を設定水準から生成し、希少確率を有効桁表示する。90%/95%の新規生成HTMLと数値表を検証する
- [x] 5.3 用語集冒頭のN比例を一般化した説明を修正し、両次元で旧表現が残らないことを検証する
- [x] 5.4 関連回帰・OpenSpec検証を実行し、検証結果とブラウザ操作の証拠限界を記録する
